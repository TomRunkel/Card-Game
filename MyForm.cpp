...
//***************** INITIATING CARDS ******************
	...
	Cards::Card Keulentraeger("Keulenträger", 0, 1, 2);
	...
	Cards::Card Heilerin("Heilerin", 0, 2, 20);
				Heilerin.addTraits(NAME_TRAIT7);
	Cards::Card Meuchler("Meuchler", 0, 1, 21);
...
//*****************************************************
	...
	for (int i = 0; i < 3; i++) DeckA.push_back(Saboteur);
	...
	for (int i = 0; i < 3; i++) DeckB.push_back(Elfenreiter);
	for (int i = 0; i < 3; i++) DeckB.push_back(Meuchler);
	for (int i = 0; i < 2; i++) DeckB.push_back(Bannertraeger);
	for (int i = 0; i < 2; i++) DeckB.push_back(Lich);
